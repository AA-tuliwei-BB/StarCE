...
def load_imdb():
    imdb_dir = imdb_data_directory
    if not imdb_dir.endswith('/'):
        imdb_dir += '/'
    aka_name = pd.read_csv(imdb_dir + "aka_name.csv", quotechar = "\"", escapechar="\\", 
                       names=["id", "person_id", "name", "imdb_index", "name_pcode_cf", "name_pcode_nf",
                              "surname_pcode", "md5sum"], low_memory=False)
    aka_title = pd.read_csv(imdb_dir + "aka_title.csv", quotechar = "\"", escapechar="\\",
    ...
    cast_info = pd.read_csv(imdb_dir + "cast_info.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "person_id", "movie_id", "person_role_id", "note", "nr_order", "role_id"], low_memory=False)
    char_name = pd.read_csv(imdb_dir + "char_name.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "name", "imdb_index", "imdb_id", "name_pcode_nf", "surname_pcode", "md5sum"], low_memory=False)
    comp_cast_type = pd.read_csv(imdb_dir + "comp_cast_type.csv", quotechar = "\"", escapechar="\\",
                           names=["id","kind"], low_memory=False)
    company_name = pd.read_csv(imdb_dir + "company_name.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "name", "country_code", "imdb_id", "name_pcode_nf", "name_pcode_sf", "md5sum"], low_memory=False)
    company_type = pd.read_csv(imdb_dir + "company_type.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "kind"], low_memory=False)
    complete_cast = pd.read_csv(imdb_dir + "complete_cast.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "movie_id", "subject_id", "status_id"], low_memory=False)
    info_type = pd.read_csv(imdb_dir + "info_type.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "info"], low_memory=False)
    keyword = pd.read_csv(imdb_dir + "keyword.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "keyword", "phonetic_code"], low_memory=False)
    kind_type = pd.read_csv(imdb_dir + "kind_type.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "kind"], low_memory=False)
    link_type = pd.read_csv(imdb_dir + "link_type.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "link"], low_memory=False)
    movie_companies = pd.read_csv(imdb_dir + "movie_companies.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "movie_id", "company_id", "company_type_id", "note"], low_memory=False)
    movie_info = pd.read_csv(imdb_dir + "movie_info.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "movie_id", "info_type_id", "info", "note"], low_memory=False)
    movie_info_idx = pd.read_csv(imdb_dir + "movie_info_idx.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "movie_id", "info_type_id", "info", "note"], low_memory=False)
    movie_keyword = pd.read_csv(imdb_dir + "movie_keyword.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "movie_id", "keyword_id"], low_memory=False)
    movie_link = pd.read_csv(imdb_dir + "movie_link.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "movie_id", "linked_movie_id", "link_type_id"], low_memory=False)
    name = pd.read_csv(imdb_dir + "name.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "name", "imdb_index", "imdb_id", "gender", "name_pcode_cf",
                                  "name_pcode_nf", "surname_pcode", "md5sum"], low_memory=False)
    person_info = pd.read_csv(imdb_dir + "person_info.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "person_id", "info_type_id", "info", "note"], low_memory=False)
    role_type = pd.read_csv(imdb_dir + "role_type.csv", quotechar = "\"", escapechar="\\",
                           names=["id", "role"], low_memory=False)
    title = pd.read_csv(imdb_dir + "title.csv", quotechar = "\"", escapechar="\\",
...
def load_stats():
    stats_dir = stats_data_directory
    if not stats_dir.endswith('/'):
        stats_dir += '/'
    badges = pd.read_csv(stats_dir + "badges.csv")
    comments = pd.read_csv(stats_dir + "comments.csv")
    postHistory = pd.read_csv(stats_dir + "postHistory.csv")
    postLinks = pd.read_csv(stats_dir + "postLinks.csv")
    posts = pd.read_csv(stats_dir + "posts.csv")
    tags = pd.read_csv(stats_dir + "tags.csv")
    users = pd.read_csv(stats_dir + "users.csv")
    votes = pd.read_csv(stats_dir + "votes.csv")
