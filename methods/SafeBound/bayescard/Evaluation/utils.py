...
def parse_query(query_str, schema):
    ...
    if identifier_token_length == 3:
        # Format: table alias (e.g., "badges b")
        ...
        # Format: table AS alias (e.g., "badges AS b")
        tables = [(token[0].value, token[4].value) for token in identifiers if
                  isinstance(token, sqlparse.sql.Identifier)]
    else:
        # No alias: table name is used as both table and alias
    ...
    for comparison in comparisons:
        ...
        if len(left.tokens) == 1:
            ...
            if isinstance(right, sqlparse.sql.Identifier):
                # Handle both single attribute (len=1) and fully qualified (len=3: table.attr)
                if len(right.tokens) == 1:
                ...
                elif (len(right.tokens) == 3 and right.tokens[1].value == '.' and 
                      right.tokens[0].ttype in (sqlparse.tokens.Name, None)):
                    # Fully qualified: alias.attribute (not a cast like 'value'::type)
                    right_part = alias_dict[right.tokens[0].value] + '.' + right.tokens[2].value
                else:
                    # Not a table.attribute identifier (e.g., '2014-08-02'::timestamp)
                    # Treat as WHERE condition value
                ...
                if left_part + ' = ' + right_part in schema.relationship_dictionary.keys():
                    ...
                    # Try to find common target (indirect relationship through a shared primary key)
                    # e.g., badges.UserId = comments.UserId -> both point to users.Id
                    left_table, left_attr = left_part.rsplit('.', 1)
                    right_table, right_attr = right_part.rsplit('.', 1)
                    
                    # Find if both attributes reference the same primary key
                    common_target = None
                    for rel_key in schema.relationship_dictionary.keys():
                        # Check if left_part points to some table.pk
                        if rel_key.startswith(left_part + ' = '):
                            target = rel_key.split(' = ', 1)[1]
                            # Check if right_part also points to the same target
                            if right_part + ' = ' + target in schema.relationship_dictionary.keys():
                                common_target = target
                                break
                            elif target + ' = ' + right_part in schema.relationship_dictionary.keys():
                                common_target = target
                                break
                    
                    if common_target:
                        # Extract target table name
                        target_table = common_target.rsplit('.', 1)[0]
                        # Add target table to alias_dict if not present (before calling add_join_condition)
                        if target_table not in alias_dict:
                            alias_dict[target_table] = target_table
                        # Add both relationships through the common target
                        # (add_join_condition will automatically add tables to query.table_set)
                        query.add_join_condition(left_part + ' = ' + common_target)
                        query.add_join_condition(right_part + ' = ' + common_target)
                    else:
                        raise AssertionError(f"Relationship unknown: {left_part} = {right_part}")
            ...
            if isinstance(right, sqlparse.sql.Identifier):
                # Handle both single attribute (len=1) and fully qualified (len=3: table.attr)
                if len(right.tokens) == 1:
                    # Single attribute, need to find matching table
                    right_attribute = right.tokens[0].value
                    right_table_name = _find_matching_table(right_attribute, schema, alias_dict)
                    right_part = right_table_name + '.' + right_attribute
                elif (len(right.tokens) == 3 and right.tokens[1].value == '.' and 
