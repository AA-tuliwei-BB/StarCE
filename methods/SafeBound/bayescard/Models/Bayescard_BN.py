...
class Bayescard_BN(BN_Single):
    ...
    def _parse_conditions(self, conditions, group_by_columns=None, group_by_tuples=None):
        ...
        for (table, condition) in conditions:
            ...
            if self.meta_types[attribute_index] == MetaType.DISCRETE:
                ...
                def normalize_literal_key(raw_literal):
                    literal_key = raw_literal
                    if literal_key in val_dict:
                        return literal_key
                    try:
                        int_key = int(raw_literal)
                        if int_key in val_dict:
                            return int_key
                    except (ValueError, TypeError):
                        pass
                    try:
                        float_key = float(raw_literal)
                        if float_key in val_dict:
                            return float_key
                    except (ValueError, TypeError):
                        pass
                    return literal_key
                ...
                if '=' in condition:
                    ...
                    literal = normalize_literal_key(literal)
                    ...
                    literal_list = _literal_list(condition)
                    literal_list = [normalize_literal_key(literal) for literal in literal_list]
    ...
    def init_inference_method(self, algorithm=None):
        ...
        if self.infer_algo == "exact-jit":
            ...
            from pgympy.inference import VariableEliminationJIT
            ...
            assert self.algorithm == "chow-liu", "Currently JIT only supports CLT"
            from pgympy.inference import VariableEliminationJIT_torch
            cpds, topological_order, topological_order_node = self.align_cpds_in_topological()
            self.cpds = cpds
            self.topological_order = topological_order
            self.topological_order_node = topological_order_node
            ...
            from pgympy.inference import VariableElimination
            ...
            try:
                ...
                from pgympy.inference import BeliefPropagation
            ...
            from pgympy.sampling import BayesianModelSampling
    ...
    def continuous_range_map(self, col, range):
        ...
        def binary_search(i, j):
            ...
            if left >= interval.right:
                # Ensure search window shrinks even when m == i.
                next_i = m + 1 if m == i else m
                return binary_search(next_i, j)
            elif right <= interval.left:
                # Ensure search window shrinks even when m == j.
                next_j = m - 1 if m == j else m
                return binary_search(i, next_j)
            else:
                return m
