...
def create_binned_cols(db_conn_kwargs, bins, equivalent_keys, sampling_percentage, sampling_type):
    ...
    for bkey, binvals in bins.items():
        for key in equivalent_keys[bkey]:
            ...
            if new_table not in created_tables:
            ...
            # Create index on the key column to speed up updates
            newkey = key[key.find(".") + 1:]
            idx_name = f"idx_{new_table}_{newkey}".replace(".", "_")
            create_idx_sql = f"CREATE INDEX IF NOT EXISTS {idx_name} ON {new_table}({newkey});"
