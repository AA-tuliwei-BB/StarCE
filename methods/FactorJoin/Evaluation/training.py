...
def train_one_imdb(data_path, model_folder, n_dim_dist=1, bin_size=None, bucket_method="fixed_start_key",
                   ...
                   materialize_sample=False, dataset='imdb'):
    """
    ...
    if bin_size is None:
        ...
        bin_size = "costumized"
    model_path = os.path.join(model_folder, f"model_{dataset}_{bin_size}.pkl")
    pickle.dump(be, open(model_path, 'wb'), pickle.HIGHEST_PROTOCOL)
    pickle.dump(be, open(model_path, 'wb'), pickle.HIGHEST_PROTOCOL)
    print(f"models save at {model_path}")
    ...
    if prepare_sample:
        ...
        # Create additional indexes for common predicates to speed up materialization
        import psycopg2 as pg
        con = pg.connect(db_conn_kwargs)
        cursor = con.cursor()
        print("Creating additional indexes for predicates...")
        index_sqls = [
            "CREATE INDEX IF NOT EXISTS idx_title_ss_prod_year ON title_ss1d0(production_year);",
            "CREATE INDEX IF NOT EXISTS idx_title_ss_kind_id ON title_ss1d0(kind_id);",
            "CREATE INDEX IF NOT EXISTS idx_aka_title_ss_kind_id ON aka_title_ss1d0(kind_id);",
            "CREATE INDEX IF NOT EXISTS idx_movie_companies_ss_ctid ON movie_companies_ss1d0(company_type_id);",
            "CREATE INDEX IF NOT EXISTS idx_movie_info_idx_ss_itid ON movie_info_idx_ss1d0(info_type_id);",
            "CREATE INDEX IF NOT EXISTS idx_movie_info_ss_itid ON movie_info_ss1d0(info_type_id);",
            "CREATE INDEX IF NOT EXISTS idx_movie_keyword_ss_kid ON movie_keyword_ss1d0(keyword_id);"
        ]
        for sql in index_sqls:
            try:
                ...
                print(f"Index creation failed (expected if table not exist): {e}")
                con.rollback()
        con.close()
