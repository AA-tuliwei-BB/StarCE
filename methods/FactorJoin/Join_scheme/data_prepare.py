...
from Schemas.imdb.schema import gen_imdb_schema, gen_job_light_imdb_schema, gen_jobm_schema
...
def convert_str_type_to_int_for_job_light_ranges(schema, query_path, target_dir):
    ...
    for table in schema.table_dictionary:
        ...
        # IMDB CSV files use comma separator with backslash escape and double-quote
    ...
    for query in sql_queries:
        ...
        matches = re.findall(pattern, query)
        for alias, operator, start_year, end_year in matches:
            series_year_str = start_year + '-' + end_year
            if start_year == "????":
                start_year = str(0)
            query = query.replace(f"{alias}.series_years{operator}'{series_year_str}'", f"{alias}.series_years{operator}{start_year}")
...
def read_table_csv(table_obj, csv_seperator=',', db_name="stats"):
    ...
    if db_name == "stats":
        for attribute in df_rows.columns:
            if "Date" in attribute and (
                df_rows[attribute].dtype == "object" or np.issubdtype(df_rows[attribute].dtype, np.str_)
            ):
                df_rows[attribute] = [timestamp_transorform(value) for value in df_rows[attribute].values]
    ...
    for table_obj in schema.tables:
        ...
        if table_name in actual_data:
            ...
            if dataset == "stats":
                ...
                # For imdb-light: check if CSV is preprocessed (has header) or raw (no header)
                # Preprocessed CSVs have headers like "title.id,title.kind_id,..."
                # Read first line to check
                with open(table_obj.csv_file_location, 'r') as f:
                    first_line = f.readline().strip()
                
                # If first line contains table_name prefix, it's preprocessed with header
                if table_obj.table_name + '.' in first_line:
                    # Preprocessed CSV with header
                    df_rows = pd.read_csv(table_obj.csv_file_location)
                    # Already has correct column names and irrelevant attributes removed
                else:
                    # Raw IMDB CSV without header
                    df_rows = pd.read_csv(table_obj.csv_file_location, header=None, escapechar='\\', 
                                        encoding='utf-8', quotechar='"', sep=',')
                    df_rows.columns = [table_obj.table_name + '.' + attr for attr in table_obj.attributes]
                    for attribute in table_obj.irrelevant_attributes:
                        df_rows = df_rows.drop(table_obj.table_name + '.' + attribute, axis=1)
                df_rows = df_rows.apply(pd.to_numeric, errors="ignore")
        for attr in df_rows.columns:
            if attr in all_keys:
                ...
                # Skip null value calculation for non-numeric columns (strings)
                if temp.dtype == 'object' or np.issubdtype(temp.dtype, np.str_):
            ...
            if attr in all_keys:
                ...
                if attr not in null_values[table_name]:
                    ...
                    if temp.dtype == 'object' or np.issubdtype(temp.dtype, np.str_):
                        ...
                        null_values[table_name][attr] = np.nanmin(temp) - 100
                if null_values[table_name][attr] is not None:
                    temp[np.isnan(temp)] = null_values[table_name][attr]
        data[table_name] = df_rows
...
def process_imdb_data(data_path, model_folder, n_bins, bucket_method, sample_size=1000000, save_bucket_bins=False,
    ...
    for PK in equivalent_keys:
        ...
        for K in equivalent_keys[PK]:
            ...
            group_sample_rate[K] = sample_rate[K]
        curr_n_bins = n_bins[PK] if isinstance(n_bins, dict) else n_bins
        if bucket_method == "greedy":
            _, optimal_bucket = greedy_bucketize(group_data, sample_rate, curr_n_bins, primary_keys, True)
        elif bucket_method == "sub_optimal":
            optimal_bucket = sub_optimal_bucketize(group_data, group_sample_rate, n_bins=curr_n_bins,
                                                      primary_keys=primary_keys, return_data=False)
        elif bucket_method == "naive":
            optimal_bucket = naive_bucketize(group_data, sample_rate, curr_n_bins, primary_keys, False)
        elif bucket_method == "fixed_start_key":
            start_key = get_start_key(list(group_data.keys()), table_key_count_var, primary_keys)
            print(start_key)
            optimal_bucket = fixed_start_key_bucketize(start_key, group_data, group_sample_rate, n_bins=curr_n_bins,
                                                      primary_keys=primary_keys, return_data=False)
        else:
            assert False, f"unrecognized bucketization method: {bucket_method}"
