...
class Bound_ensemble:
    ...
    def eliminate_one_key_group(self, tables, key_group, factors, relevant_keys):
        ...
        for table in eliminated_tables:
            ...
            if pdf.ndim == 1:
                ...
                for axis in range(pdf.ndim):
                    ...
                    all_modes_eliminated.append(np.minimum(bin_modes, marginal))
        if rest_group:
        ...
        for i in range(rest_group_cardinalty):
            ...
            for table in rest_group_tables:
                ...
                rest_group_modes_eliminated.append(modes_slice)
    ...
    def get_cardinality_bound_one(self, query_str, query_name=None, parsed=None):
        if parsed is not None:
            tables_all, table_queries, join_cond, join_keys = parsed
        else:
        ...
        if self.bns is not None:
            ...
            for alias, keys in join_keys.items():
                if alias in tables_all:
                    table = tables_all[alias]
                    if table in new_join_keys:
                        new_join_keys[table] = new_join_keys[table] | keys
                    else:
                        new_join_keys[table] = set(keys)
            join_keys = new_join_keys
            new_cond_factors = {}
            for alias in sorted(conditional_factors.keys(),
                                key=lambda a: len(alias_keys_orig.get(a, set())),
                                reverse=True):
                if alias in tables_all:
                    table = tables_all[alias]
