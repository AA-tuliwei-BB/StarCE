...
def process_condition(cond, tables_all=None):
    ...
    pattern = r"\b[a-zA-Z_0-9]+\.[a-zA-Z_0-9]+\b"
    if re.match(pattern, right):
        table2 = right.split(".")[0].strip()
        # Check if table2 is a valid table name (exists in tables_all or is not a pure number)
        if tables_all and table2 in tables_all:
        ...
        elif not tables_all and not table2.isdigit():
            # If tables_all is not provided, use the alias as-is (for backward compatibility)
            join = True
            join_keys[table1] = left
            join_keys[table2] = right
            return table1 + " " + table2, cond, join, join_keys
