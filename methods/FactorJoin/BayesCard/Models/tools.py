...
def discretize_series(series: pd.Series, n_mcv, n_bins, is_continous=False, continuous_bins=None, drop_na=True,
    ...
    if is_continous:
        ...
        # Filter out NaN values before sorting (qcut may return NaN for missing values)
        intervals = [x for x in temp.unique() if isinstance(x, pd.Interval)]
    ...
    value_counts = non_null_s.value_counts()
    ...
    n_least_common = non_null_s[~non_null_s.isin(most_common_vals)].nunique()
    n_bins = min(n_least_common, n_bins)
    if n_least_common > 0:
        #encoding the least common string category to int category
        n_distinct, bin_width, nl_encoding = categorical_qcut(
            non_null_s[~non_null_s.isin(most_common_vals)], n_bins, val
        )
        encoding.update(nl_encoding)
        
    #map the original value to encoded value
    temp = series.copy()
    for i in non_null_s.unique():
        temp[s == i] = encoding[i]
    del s
    
    if drop_na and has_null:
        null_encoding = n_mcv + n_bins + 1
        temp = temp.fillna(null_encoding)
        null_key = null_value if null_value is not None else "__null__"
        encoding[null_key] = null_encoding
