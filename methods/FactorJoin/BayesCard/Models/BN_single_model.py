...
class BN_Single():
    ...
    def get_attr_type(self, dataset, id_attributes=[], threshold=1000):
        ...
        for col in dataset.columns:
            if col in id_attributes:
                ...
                continue
            # Check if column is string type (object or string dtype)
            if dataset[col].dtype == 'object' or np.issubdtype(dataset[col].dtype, np.str_):
                attr_type[col] = 'categorical'
                continue
    ...
    def learn_model_structure(self, dataset, nrows=None, id_attributes=[], attr_type=None, rows_to_use=500000,
        ...
        # Check if pomegranate has the new API (>= 0.13.0 and < 0.14.0)
        try:
            ...
            # Use old API for pomegranate < 0.13.0 or >= 0.14.0
