...
class Bayescard_BN(BN_Single):
    ...
    def continuous_range_map(self, col, range):
        ...
        if len(query) == 0:
            # No bins overlap with the query range - return None instead of empty list
            return None, None
            ...
            if self.attr_type[attr] == 'continuous':
                if coverage is None:
                    ...
                    if type(query[attr]) in (tuple, list):
                        ...
                        val0 = flatten_value(query[attr][0])
                        val1 = flatten_value(query[attr][1])
                        
                        if val0 is None or val1 is None:
                            return None, None
                        
                        l = max(self.domain[attr][0], val0)
                        r = min(self.domain[attr][1], val1)
                    else:
                ...
                if (isinstance(query[attr], tuple) and len(query[attr]) == 2 and 
                    ...
                    try:
                        ...
                        max_val = float(query[attr][1]) if isinstance(query[attr][1], str) else query[attr][1]
                    except (ValueError, TypeError):
                        return None, None
                    ...
                    if self.null_values is None or len(self.null_values) == 0 or attr not in self.null_values:
                        ...
                        for val in self.encoding[attr]:
                            try:
                                val_num = float(val) if isinstance(val, str) else val
                                if val != self.null_values[attr] and min_val <= val_num <= max_val:
                                    query_list.append(val)
                            except (ValueError, TypeError):
                                continue
        ...
        if self.infer_algo == "exact-jit" or self.infer_algo == "exact-jit-torch":
            ...
            if query is None or n_distinct is None:
                if len(id_attrs) == 1:
                    ...
                    return id_attrs, np.zeros((self.bin_size[id_attrs[0]], self.bin_size[id_attrs[1]]))
                else:
                    return None, None
