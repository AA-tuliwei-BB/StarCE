...
#define JOIN_EST_KEY_MAX 256
#define JOIN_EST_MAX_ENTRIES 2000

/* key-value join estimate entry (内容匹配注入) */
typedef struct JoinEstKV {
    char key[JOIN_EST_KEY_MAX];
    double value;
} JoinEstKV;

static double card_ests[CARD_EST_QUERY_NUM] = {0.0};
static JoinEstKV join_est_entries[JOIN_EST_MAX_ENTRIES];
static int  join_est_entry_count = 0;
int         query_no = 0;
int         join_est_no = 0;
    ...
    if (fp == NULL)
        ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("cannot open card estimate file \"%s\"", filename)));
    double card_est;
    int query_cnt = 0;
...
read_from_fspn_join_estimate(const char* filename){
    ...
    if (fp == NULL)
        ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("cannot open join estimate file \"%s\"", filename)));
    char line[JOIN_EST_KEY_MAX + 64];
    int cnt = 0;

    while (fgets(line, sizeof(line), fp) != NULL && cnt < JOIN_EST_MAX_ENTRIES){
        /* 解析 "{aliases}\tvalue" 格式 */
        char *tab = strchr(line, '\t');
        if (tab == NULL){
            elog(WARNING, "skip malformed line in join estimate file: %s", line);
            continue;
        }
        *tab = '\0';
        char *key = line;
        double value = atof(tab + 1);

        /* 去掉 key 末尾的换行 */
        size_t key_len = strlen(key);
        if (key_len > 0 && key[key_len-1] == '\r') key[key_len-1] = '\0';

        strncpy(join_est_entries[cnt].key, key, JOIN_EST_KEY_MAX - 1);
        join_est_entries[cnt].key[JOIN_EST_KEY_MAX - 1] = '\0';
        join_est_entries[cnt].value = value;
        cnt++;
    }

    join_est_entry_count = cnt;
    fclose(fp);

    /* debug: log loaded estimates */
    {
        FILE *dbg = fopen("injection_debug.txt", "a+");
        fprintf(dbg, "[load] %s: %d entries\n", filename, cnt);
        for (int i = 0; i < cnt; i++)
            fprintf(dbg, "  %s -> %.0f\n", join_est_entries[i].key, join_est_entries[i].value);
        fclose(dbg);
    }
}

/*
 * 从 RelOptInfo 中提取所有基表别名，生成 join key：{alias1,alias2,...}
 * 别名统一转为小写，与 Python 侧的 _subquery_join_key 一致。
 */
static void
join_to_key(PlannerInfo *root, RelOptInfo *inner_rel, RelOptInfo *outer_rel,
            char *key, size_t key_size)
{
    /* 收集 relids 中的所有基表 */
    Relids all_relids = bms_union(inner_rel->relids, outer_rel->relids);
    int x = -1;
    char aliases[JOIN_EST_MAX_ENTRIES][NAMEDATALEN];
    int naliases = 0;

    while ((x = bms_next_member(all_relids, x)) >= 0)
    {
        if (x < root->simple_rel_array_size &&
            root->simple_rte_array[x] &&
            root->simple_rte_array[x]->eref)
        {
            char *alias = root->simple_rte_array[x]->eref->aliasname;
            strncpy(aliases[naliases], alias, NAMEDATALEN - 1);
            aliases[naliases][NAMEDATALEN - 1] = '\0';
            /* 转小写 */
            for (char *p = aliases[naliases]; *p; p++)
    ...
    /* 按字母序排序 */
    for (int i = 0; i < naliases - 1; i++)
        for (int j = i + 1; j < naliases; j++)
            if (strcmp(aliases[i], aliases[j]) > 0)
            {
                char tmp[NAMEDATALEN];
                strcpy(tmp, aliases[i]);
                strcpy(aliases[i], aliases[j]);
                strcpy(aliases[j], tmp);
            }
...
/*
 * 按 key 查找估计值。返回估计值，若未找到返回 -1.0。
 */
static double
lookup_join_estimate(const char *key)
{
    for (int i = 0; i < join_est_entry_count; i++)
    {
        if (strcmp(join_est_entries[i].key, key) == 0)
            return join_est_entries[i].value;
    }
    return -1.0;
}
					...
					JoinPathExtraData *extra)
{
	...
	{
		FILE *_fchj = fopen("injection_debug.txt", "a+");
		fprintf(_fchj, "[final_hj] parent_rows=%.0f has_param=%d par_workers=%d path_rows=%.0f\n",
				path->jpath.path.parent->rows,
				(path->jpath.path.param_info != NULL),
				path->jpath.path.parallel_workers,
				path->jpath.path.rows);
		fclose(_fchj);
	}
	...
	if (path->jpath.path.parallel_workers > 0)
	{
		...
		{
			FILE *_fchj2 = fopen("injection_debug.txt", "a+");
			fprintf(_fchj2, "[final_hj_after_par] path_rows=%.0f par_div=%.0f\n",
					path->jpath.path.rows, parallel_divisor);
			fclose(_fchj2);
		}
	}
						   ...
						   List *restrictlist)
{
	...
	/* debug: trace what value was actually set on the joinrel */
	{
		char _key[JOIN_EST_KEY_MAX];
		join_to_key(root, inner_rel, outer_rel, _key, sizeof(_key));
		FILE *_dbg = fopen("injection_debug.txt", "a+");
		fprintf(_dbg, "[set_rows] key=%s outer_rows=%.0f inner_rows=%.0f final_rows=%.0f\n",
				_key, outer_rel->rows, inner_rel->rows, rel->rows);
		fclose(_dbg);
	}
...
/*  print for sub-queries*/
    ...
    if (ml_joinest_enabled) {
        ...
        /* join_est_no==0 表示新查询开始，重新加载估计文件 */
        if (join_est_no == 0) {
            join_est_entry_count = 0;
            {
                FILE *dbg = fopen("injection_debug.txt", "a+");
                fprintf(dbg, "[new_query] join_est_no reset, will reload %s\n", ml_joinest_fname);
                fclose(dbg);
            }
        }
        ...
        if (join_est < 0) {
            /* 估计错位——key 在估计文件中找不到，说明提取/注入不一致。 */
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("join estimate key \"%s\" not found in estimate file \"%s\"",
                        key, ml_joinest_fname)));
        }

        {
            FILE *dbg = fopen("injection_debug.txt", "a+");
            fprintf(dbg, "[inject] qno=%d est_no=%d key=%s pg=%.0f injected=%.0f\n",
                    query_no, join_est_no, key, nrows, join_est);
            fclose(dbg);
        }
