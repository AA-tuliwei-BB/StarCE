...
bool RelationManager::IsAllRelLogicalGet() {
	for (idx_t i = 0; i < relations.size(); i++) {
		if (relations[i]->op.type != LogicalOperatorType::LOGICAL_GET) {
			return false;
		}
	}
	return true;
...
std::string RelationManager::GetFilterString(idx_t idx) {
	if (idx >= relations.size()) {
		throw std::out_of_range("Index out of range for relations");
	}
	if (relations[idx]->op.type != LogicalOperatorType::LOGICAL_GET
		&& relations[idx]->op.type != LogicalOperatorType::LOGICAL_FILTER) {
		throw std::invalid_argument("Filter string can only be retrieved for LogicalGet or LogicalFilter operators");
	}
	std::string result;
	bool first_item = true;
	if (relations[idx]->op.type == LogicalOperatorType::LOGICAL_FILTER) {
		// const auto &filter_op = relations[idx]->op.Cast<LogicalFilter>();
		// std::cerr << filter_op.ParamsToString()["Expressions"] << std::endl;
		// std::cerr << filter_op.ToString() << std::endl;
		std::cerr << relations[idx]->op.Cast<LogicalFilter>().ParamsToString()["Expressions"] << std::endl;
		std::cerr << relations[idx]->op.Cast<LogicalFilter>().ToString() << std::endl;
		int i = 0;
		++i;
		...
		// 	}
		// 	first_item = false;
		// 	result += filter_op.expressions.at(i)->ToString();
		// }
		// const auto & table_filters = relations[idx]->op.children[0]->Cast<LogicalGet>().table_filters;
		// const auto & names = relations[idx]->op.Cast<LogicalGet>().names;
		...
		// 		}
		// 		first_item = false;
		// 		result += filter->ToString("__table_name." + names[column_index]);
		// 	}
		// }
	} else {
		const auto & table_filters = relations[idx]->op.Cast<LogicalGet>().table_filters;
		const auto & names = relations[idx]->op.Cast<LogicalGet>().names;
		for (auto &kv : table_filters.filters) {
			auto &column_index = kv.first;
			auto &filter = kv.second;
...
void RelationManager::ResetCardinality(starce::StatisticManager& sm) {
	for (idx_t i = 0; i < relations.size(); i++) {
		relations[i]->op.SetEstimatedCardinality(sm.GetTableCard(i));
		relations[i]->stats.cardinality = relations[i]->op.estimated_cardinality;
	}
}
