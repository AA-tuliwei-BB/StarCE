...
bool QueryGraphManager::Build(JoinOrderOptimizer &optimizer, LogicalOperator &op, bool &can_use_starce) {
	// have the relation manager extract the join relations and create a reference list of all the
	// filter operators.
	can_use_starce = false;
	...
	// TAG-STARCE
	// can_use_starce = relation_manager.IsAllRelLogicalGet();
	auto &sm = starce::StatisticManager::GetInstance();
	can_use_starce = sm.EnableStarCE;
	if (can_use_starce) {
		sm.PrepareEstimate();
		auto stats = relation_manager.GetRelationStats();
		for (int i = 0; i < stats.size(); ++i) {
			...
			// sm.AddTable(i, table_name, stats[i].column_names, stats[i].cardinality, filterString);
			sm.AddTable(i, table_name, stats[i].column_names, stats[i].cardinality);
		}
		if (sm.UseSingleTableCard) {
			relation_manager.ResetCardinality(sm);
		}
	}
				...
				if (filter_info->left_set != filter_info->right_set) {
					...
					if (Disjoint(left_bindings, right_bindings)) {
						...
						if (filter_info->left_set->count == 1 && filter_info->right_set->count == 1
							...
							&& comparison.left->GetExpressionClass() == ExpressionClass::BOUND_COLUMN_REF
							&& comparison.right->GetExpressionClass() == ExpressionClass::BOUND_COLUMN_REF) {
							sm.AddPredicate(filter_info->left_set->relations[0],
							                filter_info->right_set->relations[0],
							                filter_info->left_binding.column_index,
							                filter_info->right_binding.column_index);
						}
					}
				}
			}
